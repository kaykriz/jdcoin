...
uint256 hashGenesisBlock("0x0bd61146fa26844507e2af0482fb11ffeb6849ea5675338f8553955e29991f0c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jdcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jdcoin Signed Message:\n";
...
{
    // Jdcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Jdcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Jdcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Jdcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jdcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jdcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x789e1dbf98b279d159e0175e3f77b21877b3a6ea12380aa80d30e83468983a92");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 29/03/2018 Visit to China Bolsters Hand Of North Korea";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c28690a283354cb6346ae575e6b82bc71ad4be68d0e1283df0f211f76648623bf17e7c9b7d9a13adf0b64e1659c6aa5baa7fa68bfd330d3d9264bc99d5df446f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522399673;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088363156;

        if (fTestNet)
        {
            block.nTime    = 1522399638;
            block.nNonce   = 386627829;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x71893b706f51110caca1b1f3bf7f36c3bb52779a98df90d3b473decc052fef07"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc3, 0xb3, 0xd3 }; // Jdcoin: uses random values.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JdcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JdcoinMiner(CWallet *pwallet)
{
    printf("JdcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running JdcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JdcoinMiner, pwallet));
}
